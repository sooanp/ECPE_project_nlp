...
pf_tr_ld = DataLoader(EmotionDataset(Xp_tr, yp_tr, tokenizer, max_len),
                        batch_size=batch_size, shuffle=True)
pf_te_ld = DataLoader(EmotionDataset(Xp_te, yp_te, tokenizer, max_len),
...
model_f = LSTMClassifier(vocab_size,128,128,2,pad_idx).to(device)
optimizer   = AdamW(model_f.parameters(), lr=1e-3)
loss_fn_weight = nn.CrossEntropyLoss(weight=weights)

for ep in range(epochs):
    lf = train_epoch(model_f, pf_tr_ld, optimizer, loss_fn)
    print(f"Filter Epoch{ep+1} Loss={lf:.3f}")

pf_p, pf_t = eval_model(model_f, pf_te_ld)
print("==Filter==\n", classification_report(pf_t, pf_p, target_names=['neg','pos']))
print("ConfMat\n", confusion_matrix(pf_t, pf_p))